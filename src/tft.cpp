...
void TFT::init() {
    ...
    writeCommand(ST7735_FRMCTR1);   // Frame rate ctrl - normal mode
    SPI.write(0x01);                // Rate = fosc/(1x2+40) * (LINE+2C+2D)
    SPI.write(0x2C);
    SPI.write(0x2D);
    ...
    writeCommand(ST7735_FRMCTR3);   // Frame rate ctrl - partial mode
    SPI.write(0x01);                // Dot inversion mode
    SPI.write(0x2C);
    SPI.write(0x2D);
    SPI.write(0x01);
    SPI.write(0x2C);                // Line inversion mode
    SPI.write(0x2D);

    writeCommand(ST7735_INVCTR);    // Display inversion ctrl
    SPI.write(0x07);                // No inversion

    writeCommand(ST7735_PWCTR1);    // Power control
    SPI.write(0xA2);
    SPI.write(0x02);                // -4.6V
    SPI.write(0x84);                // AUTO mode
    ...
    writeCommand(ST7735_INVOFF);    // Display Inversion Off
    ...
    writeCommand(ST7735_MADCTL);    // Memory Data Access Control
    SPI.write(0xC0);
    ...
    writeCommand(ST7735_GMCTRN1);   //  Gamma (‘-’polarity) Correction Characteristics Setting
    SPI.write(0x03);
    SPI.write(0x1D);
    SPI.write(0x07);
    SPI.write(0x06);
...
void TFT::begin(uint8_t CS, uint8_t DC){
    String info="";
    TFT_CS=CS; TFT_DC=DC;
...
void TFT::setAddrWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
     ...
     SPI.write(x >> 8);
     SPI.write(x & 0xFF);     // XSTART
     w=x+w-1;
     SPI.write(w >> 8);
     SPI.write(w & 0xFF);     // XEND

     writeCommand(ST7735_PASET); // Row addr set
     SPI.write(y >> 8);
     SPI.write(y & 0xFF);     // YSTART
     h=y+h-1;
     SPI.write(h >> 8);
     SPI.write(h & 0xFF);     // YEND
...
void TFT::startBitmap(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
    startWrite();
        writeCommand(ST7735_MADCTL);
        if(_rotation==0){SPI.write(MADCTL_MX | MADCTL_BGR);}
        if(_rotation==1){SPI.write(MADCTL_MX | MADCTL_MV | MADCTL_MY |MADCTL_BGR);}
        if(_rotation==2){SPI.write(MADCTL_MH | MADCTL_MY | MADCTL_BGR);}
        if(_rotation==3){SPI.write(MADCTL_MV | MADCTL_BGR);}
...
void TFT::startJpeg() {
    startWrite();
        writeCommand(ST7735_MADCTL);
        if(_rotation==0){SPI.write(MADCTL_MX | MADCTL_MY | MADCTL_BGR);}
        if(_rotation==1){SPI.write(MADCTL_MY | MADCTL_MV | MADCTL_BGR);}
        if(_rotation==2){SPI.write(MADCTL_BGR);}
        if(_rotation==3){SPI.write(MADCTL_MX | MADCTL_MV | MADCTL_BGR);}
    endWrite();
}
...
bool TFT::setCursor(uint16_t x, uint16_t y) {
    ...
    SPI.write(x >> 8);
    SPI.write(x & 0xFF);
    writeCommand(ST7735_RAMWR); //Column Start
    writeCommand(0x2B);
    SPI.write(y >> 8);
    SPI.write(y & 0xFF);
    writeCommand(ST7735_RAMWR); //Row Start
    _curX=x;
    _curY=y;
...
boolean TFT::drawBmpFile(fs::FS &fs, const char * path, uint16_t x, uint16_t y, uint16_t maxWidth, uint16_t maxHeight, uint16_t offX, uint16_t offY){
    ...
    File file = fs.open(path); // @suppress("Abstract class cannot be instantiated")
    if(!file){
        return false;
    }
...
int JPEGDecoder::decodeSdFile(File jpgFile) { // @suppress("Abstract class cannot be instantiated")
    g_pInFileSd = jpgFile;
